import{f as B,a as F,r as _}from"./assets/modal-exercise-Cx8ztaUI.js";import{a as K,i as y}from"./assets/vendor-D2ogNlHo.js";const f="/project-17112025/assets/icons-not-min-DVVFTTHI.svg";function D(t,e){const s=t.map(({filter:a,imgURL:i,name:n})=>`
      ...
      `).join("");e.innerHTML=s}function T(t,e){const s=t.map(({_id:a,name:i,target:n,bodyPart:r,burnedCalories:l,rating:u})=>`
  ...
  `,W=u+Q+U;s.innerHTML=W}function O(t,e,s){const a=t.map(i=>`
    ...
    `,t.style.position="relative",t.appendChild(e)),e.classList.remove("hidden")}function q(t){if(!t)return;const e=t.querySelector(".local-loader-wrapper");e&&e.classList.add("hidden")}const C=["Muscles","Body parts","Equipment"],Y=375,j=document.documentElement.clientWidth<=Y,A=document.querySelector("span.slash"),V=document.querySelector("p.current-category"),G=document.querySelector("p.warning"),H=document.querySelector("ul.filters-list"),$=document.querySelector("ul.block-categories-list"),k=document.querySelector("ul.exercises-list"),d=document.querySelector("ul.pagination-controls-list"),x=document.querySelector("form.search-form"),z=x.querySelector(".search-input"),P=document.querySelector("button.clear-btn"),m=document.getElementById("categories-section");let v=C[0],g="",h="",c=1,L=1,N=j?9:12,o=1,p=1,I=j?8:10;x.addEventListener("input",t=>{h=t.target.value,h.length?P.classList.remove("visually-hidden"):P.classList.add("visually-hidden")});x.addEventListener("submit",async t=>{t.preventDefault(),M(m);try{const e=await B(v,g,h,o,I),{results:s,page:a,totalPages:i}=e;p=i,o=a,d.innerHTML="",T(s,k),b(p,o,d)}catch(e){y.error({icon:"",position:"topRight",message:e.message})}finally{q(m)}});P.addEventListener("click",async()=>{h="",z.value="",P.classList.add("visually-hidden"),T(results,k),b(p,o,d),R()});async function R(){$.innerHTML="",k.innerHTML="",d.innerHTML="",M(m);try{const t=await F(v,c,N),{results:e,page:s,totalPages:a}=t;L=a,c=s,D(e,$),b(L,c,d)}catch(t){G.classList.remove("visually-hidden"),y.error({icon:"",position:"topRight",message:t.message})}finally{q(m)}}H.addEventListener("click",J);function J(t){const e=t.target.closest(".filters-list-item");e&&(v=e.dataset.option,R(),O(C,v,H),x.classList.add("visually-hidden"),h="",g="",z.value="",A.classList.add("visually-hidden"),V.textContent=g)}async function X(t){const e=t.target.closest(".categories-item");if(e){M(m),g=e.dataset.name;try{const s=await B(v,g,h,o,I),{results:a,page:i,totalPages:n}=s;p=n,o=i,$.innerHTML="",d.innerHTML="",T(a,k),b(p,o,d),x.classList.remove("visually-hidden"),A.classList.remove("visually-hidden"),V.textContent=g}catch(s){y.error({icon:"",position:"topRight",message:s.message})}finally{q(m)}}}async function ee(t){const e=t.target.closest("button[data-page]");if(e){M(m);try{if(g){switch(e.dataset.page){case"beg":o=1;break;case"prev":o--;break;case"next":o++;break;case"end":o=p;break;default:o=parseInt(e.dataset.page,10)}const s=await B(v,g,h,o,I),{results:a,page:i,totalPages:n}=s;p=n,o=i,T(a,k),b(p,o,d)}else{switch(e.dataset.page){case"beg":c=1;break;case"prev":c--;break;case"next":c++;break;case"end":c=L;break;default:c=parseInt(e.dataset.page,10)}const s=await F(v,c,N),{results:a,page:i,totalPages:n}=s;L=n,c=i,D(a,$),b(L,c,d)}}catch(s){y.error({icon:"",position:"topRight",message:s.message})}finally{q(m)}}}O(C,v,H);R();$.addEventListener("click",X);d.addEventListener("click",ee);document.addEventListener("DOMContentLoaded",_);const S=document.getElementById("scroll-top");console.log(S);const te=()=>{document.body.scrollTop=0,document.documentElement.scrollTop=0},se=()=>{window.scrollY>window.innerHeight?S.classList.add("scroll-show"):S.classList.remove("scroll-show")};window.addEventListener("scroll",se);S.addEventListener("click",te);
