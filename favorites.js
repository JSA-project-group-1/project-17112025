import{e as k,a as g,i as p,o as M}from"./assets/render-functions-wNSWn-tv.js";import{i as v}from"./assets/vendor-D2ogNlHo.js";const h="favorite_workouts",L=375,S=document.documentElement.clientWidth<=L,T=1440,F=document.documentElement.clientWidth>=T;let t=[],s=1,o=1,d=S?8:F?1e3:10;const c=document.querySelector("ul.pagination-controls-list"),l=document.querySelector("ul.favorites-list");document.getElementById("favorites-blok");function m(){const a=localStorage.getItem(h);if(!a)return[];try{const e=JSON.parse(a);return Array.isArray(e)?e:[]}catch(e){return console.error("Failed to parse favorites from LS:",e),v.error({message:"Favorites data is corrupted. Clearing storage.",position:"topRight",maxWidth:600}),localStorage.removeItem(h),[]}}function I(a){localStorage.setItem(h,JSON.stringify(a))}function w(){l.innerHTML="",c.innerHTML="";try{t=m(),o=Math.max(1,Math.ceil(t.length/d));const a=(s-1)*d,e=s*d,n=t.slice(a,e);u(n,l),t.length>0&&g(o,s,c)}catch(a){v.error({icon:"",position:"topRight",message:a.message})}}function u(a,e){if(a.length>0){const r=a.map(({_id:i,name:f,target:y,bodyPart:b,burnedCalories:x})=>`
