import{e as x,a as u,i as p}from"./assets/render-functions-t7e8hK3R.js";import{i as v}from"./assets/vendor-D2ogNlHo.js";const h="favorite_workouts",k=375,M=document.documentElement.clientWidth<=k,L=1440,S=document.documentElement.clientWidth>=L;let e=[],n=1,i=1,d=M?8:S?1e3:10;const o=document.querySelector("ul.pagination-controls-list"),c=document.querySelector("ul.favorites-list");document.getElementById("favorites-blok");function m(){const t=localStorage.getItem(h);if(!t)return[];try{const a=JSON.parse(t);return Array.isArray(a)?a:[]}catch(a){return console.error("Failed to parse favorites from LS:",a),v.error({message:"Favorites data is corrupted. Clearing storage.",position:"topRight",maxWidth:600}),localStorage.removeItem(h),[]}}function T(t){localStorage.setItem(h,JSON.stringify(t))}function F(){c.innerHTML="",o.innerHTML="";try{e=m(),i=Math.max(1,Math.ceil(e.length/d));const t=(n-1)*d,a=n*d,s=e.slice(t,a);g(s,c),e.length>0&&u(i,n,o)}catch(t){v.error({icon:"",position:"topRight",message:t.message})}}function g(t,a){if(t.length>0){const s=t.map(({_id:r,name:l,target:f,bodyPart:y,burnedCalories:b})=>`
