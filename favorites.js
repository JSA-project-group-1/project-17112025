import{e as x,a as g,i as h,o as L}from"./assets/render-functions-wNSWn-tv.js";import{i as m}from"./assets/vendor-D2ogNlHo.js";const v="favorite_workouts",M=375,S=document.documentElement.clientWidth<=M,T=1440,F=document.documentElement.clientWidth>=T;let a=[],s=1,o=1,d=S?8:F?1e3:10;const c=document.querySelector("ul.pagination-controls-list"),l=document.querySelector("ul.favorites-list");document.getElementById("favorites-blok");function f(){const t=localStorage.getItem(v);if(!t)return[];try{const e=JSON.parse(t);return Array.isArray(e)?e:[]}catch(e){return console.error("Failed to parse favorites from LS:",e),m.error({message:"Favorites data is corrupted. Clearing storage.",position:"topRight",maxWidth:600}),localStorage.removeItem(v),[]}}function I(t){localStorage.setItem(v,JSON.stringify(t))}function w(){l.innerHTML="",c.innerHTML="";try{a=f(),o=Math.max(1,Math.ceil(a.length/d));const t=(s-1)*d,e=s*d,i=a.slice(t,e);u(i,l),a.length>0&&g(o,s,c)}catch(t){m.error({icon:"",position:"topRight",message:t.message})}}function u(t,e){if(t.length>0){const n=t.map(({_id:r,name:p,target:b,bodyPart:y,burnedCalories:k})=>`
